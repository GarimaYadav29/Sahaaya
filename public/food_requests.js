...
async function fetchFoodDonations(state = "", city = "", pincode = "") {
  try {
      const query = new URLSearchParams({ state, city, pincode }).toString();
      const response = await fetch(`/food_donations?${query}`);
      ...
      const container = document.getElementById('food-donations-container');
      ...
      donations.forEach(donation => {
          ...
          donationDiv.innerHTML = `
              <h3>Food Item: ${donation.food_item_name}</h3>
              <p>Quantity: ${donation.quantity}</p>
              <p>Type: ${donation.item_type}</p>
              ...
              <button onclick="showAcceptRequestForm(${donation.id}, '${donation.donor_name}')">Accept Request</button>
          `;
...
function showAcceptRequestForm(donationId, receiver_name) {
  const modal = document.getElementById('accept-request-modal');
  modal.style.display = 'block';

  // Handle form submission for accepting request
  const form = document.getElementById('accept-request-form');
