...
async function fetchbookDonations(state = "", city = "", pincode = "") {
  try {
      const query = new URLSearchParams({ state, city, pincode }).toString();
      const response = await fetch(`/book_donations?${query}`);

      if (!response.ok) {
          throw new Error('Failed to fetch book donations');
      }

      const donations = await response.json();
      const container = document.getElementById('book-donations-container');
      ...
      donations.forEach(donation => {
          ...
          donationDiv.innerHTML = `
              <h3>book name: ${donation.book_name}</h3>
              <p>genre: ${donation.genre}</p>
              <p>author: ${donation.author}</p>
              <p>quantity: ${donation.quantity}</p>
              <p>book_condition: ${donation.book_condition}</p>
              <p>age_group: ${donation.age_group}</p>
              <p>Donor Name: ${donation.donor_name}</p>
              <p>Address: ${donation.address}, ${donation.city}, ${donation.state}, ${donation.pincode}</p>
              <p>Contact: ${donation.phone}</p>
              <button onclick="showAcceptbookRequestForm(${donation.id}, '${donation.donor_name}')">Accept Request</button>
          `;
...
function showAcceptbookRequestForm(receiver_name) {
  const modal = document.getElementById('accept-book-request-modal');
  modal.style.display = 'block';

  // Handle form submission for accepting request
  const form = document.getElementById('accept-book-request-form');
  form.onsubmit = async function(event) {
      ...
      try {
          const response = await fetch('/accept_book_request', {
              method: 'POST',
...
window.onload = fetchbookDonations;
...
document.getElementById('search-button').addEventListener('click', () => {
  ...
  fetchbookDonations(state, city, pincode);
});